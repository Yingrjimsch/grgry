...
use crate::{config::config::{Config, Profile}, utils::helper::{prntln, MessageType}, utils::cmd::{create_git_cmd, run_cmd_o, run_cmd_s}};
...
pub fn quick(
    ...
    process_repos(
        ...
        |repo| {
            ...
            let has_changes = !run_cmd_o(create_git_cmd(&repo_path).arg("status").arg("--porcelain"),
                dry_run,
            ).is_empty();
            ...
            prntln(&format!("There are changes in the repository {}", repo.display()), MessageType::Neutral);

            if skip_interactive {
                return Ok(true);
            }

            let result = CustomType::<String>::new("Do you want to quicken this repo? (y)es/(n)o/(m)ore information:",)
            ...
            match result {
                Ok(choice) => match choice.to_lowercase().as_str() {
                    ...
                    "m" => {
                        ...
                        prntln(&format!("{:<10}: {}", "URL", get_remote_url(&repo_path, dry_run)), MessageType::Success);
                        prntln(&format!("{:<10}: {}", "Branch", get_current_branch(&repo_path, dry_run)), MessageType::Success);
                        Ok(false)
                    }
                    _ => unreachable!(),
                },
...
fn get_current_branch(repo_path: &str, dry_run: bool) -> String {
    run_cmd_o(create_git_cmd(repo_path).args(&["branch", "--show-current"]), dry_run)
}
...
fn execute_quick_actions(repo_path: &str, profile: &Profile, message: &str, dry_run: bool) {
    run_cmd_s(create_git_cmd(repo_path).args(&["pull", "--rebase"]), dry_run, true);
    run_cmd_s(create_git_cmd(repo_path).args(&["config", "user.name", &profile.username]), dry_run, true);
    run_cmd_s(create_git_cmd(repo_path).args(&["config", "user.email", &profile.email]), dry_run, true);
    run_cmd_s(create_git_cmd(repo_path).args(&["add", "."]), dry_run, true);
    run_cmd_s(create_git_cmd(repo_path).args(&["commit", "-m", message]), dry_run, true);
    ...
    run_cmd_s(create_git_cmd(repo_path).args(create_push_request_args(&branch, set_upstream)), dry_run, true);

    prntln(&format!("\n{} {} {} {}",
        "Successfully pushed repo into:",
        get_remote_url(repo_path, dry_run),
        "on branch",
        branch), MessageType::Success);
}
